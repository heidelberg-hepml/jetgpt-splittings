...
from src.eventgen.preprocessing import StandardPreprocessing
...
class JetGPT(nn.Module):
    def __init__(
        ...
        # accept only valid orderings
        assert (
            min(ordering) == 0 and max(ordering) == 3
        ), "ordering contains elements <0 or >3"
        assert (
            len(ordering) == 4 and len(np.unique(ordering)) == 4
        ), "ordering contains duplicate elements"
        assert ordering[-1] == 3, "mass has to come last in ordering"
        self.register_buffer("ordering", torch.tensor(ordering, dtype=torch.long))
        self.register_buffer("ordering_reverse", torch.argsort(self.ordering))
    ...
    def extra_information(
        ...
        self.preprocessing = StandardPreprocessing(
            self.pt_min, self.units, self.onshell_list, device=device, dtype=dtype_prep
        )
    ...
    def forward(self, fourmomenta, num_particles):
        ...
        # call cformer
        log_prob_kinematics = self._log_prob_particle(pcondition, x)
        log_prob_kinematics[
            ..., self.onshell_list, 3
    ...
    def _log_prob_particle(self, pcondition, x):
        pcondition = pcondition.unsqueeze(-2).repeat(1, 1, 4, 1)
        x_ordered = x[..., self.ordering][..., :-1]
        x_condition = torch.cat((torch.zeros_like(x[:, :, [0]]), x_ordered), dim=-1)
        cidx = torch.arange(4, device=x.device)[..., self.ordering]
        print(cidx)
        ccondition = self._get_cformer_condition(x_condition, pcondition, cidx)[
            ..., self.ordering_reverse, :
        ]
        is_angle = cidx == 1
        log_prob_kinematics = self.mixturemodel.log_prob(x, ccondition, is_angle)
        return log_prob_kinematics
    ...
    def _sample_particle(self, pcondition, i, device, dtype):
        ...
        cidx_full = torch.arange(4, device=device)[self.ordering][:ncomponents]
        ...
        for icomponent in range(ncomponents):
            ...
            print(icomponent, cidx, is_angle)
        ...
        xparticle = xparticle[:, 1:][:, self.ordering_reverse]
        return xparticle
