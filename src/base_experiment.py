...
from src.misc import get_device, flatten_dict
import src.logger
from src.logger import LOGGER, MEMORY_HANDLER, FORMATTER
from src.mlflow import log_mlflow
import src.networks.attention
...
class BaseExperiment:
    ...
    def __call__(self):
        ...
        if not src.logger.LOGGING_INITIALIZED:
    ...
    def init_model(self):
        ...
        LOGGER.info(
            f"Instantiated model {type(self.model).__name__} with {num_parameters} learnable parameters"
        )
    ...
    def _init_directory(self):
        ...
        if self.cfg.save_source:
            ...
            path_code = os.path.join(self.cfg.base_dir, "src")
            for root, dirs, files in os.walk(path_code):
                for file in files:
                    file_path = os.path.join(root, file)
                    zipf.write(file_path, os.path.relpath(file_path, path_code))
            zipf.close()
    ...
    def _init_logger(self):
        ...
        if src.logger.LOGGING_INITIALIZED:
        ...
        src.logger.LOGGING_INITIALIZED = True
    ...
    def train(self):
        ...
        LOGGER.info(
            ...
            f"= {self.cfg.training.iterations / len(self.loaders['trn']):.1f} epochs "
            f"on a dataset with {len(self.loaders['trn'])} batches "
        ...
        iterator = iter(cycle(self.loaders["trn"]))
            ...
            if (step + 1) % self.cfg.training.validate_every_n_steps == 0:
                ...
                if val_loss < smallest_val_loss:
                    ...
                    if patience > self.cfg.training.es_patience:
                        LOGGER.info(
                            f"Early stopping in iteration {step} = epoch {step / len(self.loaders['trn']):.1f}"
                        )
        ...
        dt = time.time() - self.training_start_time
        LOGGER.info(
            f"Finished training for {step} iterations = {step / len(self.loaders['trn']):.1f} epochs "
            f"after {dt/60:.2f}min = {dt/60**2:.2f}h"
        )
        if self.cfg.use_mlflow:
            log_mlflow("iterations", step)
            log_mlflow("epochs", step / len(self.loaders["trn"]))
    ...
    def _step(self, data, step):
        # actual update step
        try:
            loss, metrics = self._batch_loss(data, step=step)
        except IndexError:  # happens for JetGPTBootstrap
            LOGGER.warning(f"Skipping iteration {step} due to IndexError")
            return  # skip update
        ...
        with torch.no_grad():
            for data in self.loaders["val"]:
                ...
                try:
                    ...
                    LOGGER.warning(f"Skipping validation due to IndexError")
                    continue
    ...
    def _batch_loss(self, data, step=None):
        raise NotImplementedError()
